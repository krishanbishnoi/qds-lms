...
use Auth, Blade, Config, Cache, Cookie, DB, File, Hash, Input, Mail, mongoDate, Redirect, Request, Response, Session, URL, View;
 ...
 *
 * This file will render views\front\login
 */
class AdminLoginController extends BaseController
{
	...
	/**
	 * Function for display front  login page
	 *
	 * @param null
	 *
	 * @return view page.
	...
	public function login(Request $request)
	{
		...
		if (Request::isMethod('post')) {
			...
			if (!empty($formData)) {
				...
				$user = User::where($loginType, Request::get('email'))->first();

				// if ($user && $user->user_role_id == TRAINEE_ROLE_ID) {
				// dd($user);
			...
			return View::make('front.login.index');
		}
	} // end index()
	...
	{
		return View::make('front.login.forget_password');
	} // end forgetPassword()
	/**
	...
	public function resetPassword($validate_string = null)
	{
		...
		if ($validate_string != "" && $validate_string != null) {
			...
			if (!empty($userDetail)) {
				return View::make('front.login.reset_password', compact('validate_string'));
			} else {
				return Redirect::to('/')
		...
		if ($validator->fails()) {
			...
			if (!empty($userDetail)) {
				if ($userDetail->is_active == 1) {
					...
					$route_url      	=  URL::to('front/reset_password/' . $forgot_password_validate_string);
					...
					return Redirect::to('/forget_password')->with('error', trans('Your account has been temporarily disabled. Please contact frontistrator to unlock.'));
				}
			} else {
	...
	public function resetPasswordSave($validate_string = null)
	{
		...
		if ($validator->fails()) {
			return Redirect::to('reset_password/' . $validate_string)
				->withErrors($validator)->withInput();
		} else {
	...
	/**
	 * Function for logout front users
	 *
	 * @param null
	 *
