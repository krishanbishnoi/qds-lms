...
use App\Imports\importChangeDesignation;
...
{
    public $model         =     'Users';
    public $sectionName     =     'Training Manager';
    public $sectionNameSingular     =     'Training Manager';
    ...
    public function index()
    {
        ...
        $DB->where('is_deleted', 0)->where('user_role_id', MANAGER_ROLE_ID)->where("is_developer", '!=', 1);
    ...
    function save()
    {
        ...
        if (!empty($formData)) {
            ...
            if ($validator->fails()) {
                ...
                if (!empty(Request::get('allow_create_training'))) {
                    $obj->allow_create_training                      =  Request::get('allow_create_training');
                }
                if (!empty(Request::get('allow_create_test'))) {
                    $obj->allow_create_test                      =  Request::get('allow_create_test');
                }
                $obj->save();
                $userId                     =     $obj->id;
                if (!$userId) {
    ...
    public function importChangeDesignation()
    {
        $import = new importChangeDesignation();
        ...
        return redirect()->back()->with('success', 'Users Designation changed.');
    }
}
