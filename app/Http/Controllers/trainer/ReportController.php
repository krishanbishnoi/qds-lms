...
class ReportController extends BaseController
{
    ...
    public function index()
    {
        ...
        if (Auth::user()->user_role_id == 2) {
            $created_by_self_ids = $DB->where('created_by', Auth::user()->id)->pluck('id')->toArray();

            $my_assign_training_ids = BatchReport::where('created_by', Auth::user()->id)->where('id', '!=', '')->pluck('id')->toArray();
        ...
        // $DB->leftJoin('training_types', 'training_types.id', '=', 'trainings.type')->leftJoin('users', 'users.id', '=', 'trainings.user_id')->select('trainings.*', 'training_types.type as type', 'users.first_name as created_by');
        $DB->leftJoin('users', 'users.id', '=', 'batch_reports.created_by')->select('batch_reports.*', 'users.first_name as created_by')->first();
    ...
    public function add()
    {
        ...
        $trainees = User::where('is_deleted', 0)->where('user_role_id', 3)->pluck('first_name', 'id')->toArray();
        return View::make("trainer.$this->model.add", compact('trainees'));
    }// end add()
    ...
    public function edit($modelId = 0)
    {
        ...
        return View::make("trainer.$this->model.edit", compact('model', 'documents'));
    } // end edit()
    ...
    public function view($modelId = 0)
    {
        $model = BatchReport::find($modelId);
        // return $model;
        $name = $model->leftJoin('users', 'users.id', '=', 'batch_reports.created_by')->select('batch_reports.*', 'users.first_name as created_by')->first();
        ...
        return View::make("trainer.$this->model.view", compact('model', 'name'));
    }
