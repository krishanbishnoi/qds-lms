 ...
 *
 * This file will render views\trainer\login
 */
class AdminLoginController extends BaseController
{
	...
	/**
	 * Function for display trainer  login page
	 *
	 * @param null
	 *
	 * @return view page. 
	...
	public function login()
	{
		...
		if (Request::isMethod('post')) {
				...
				if ($validator->fails()) {
					...
					if (!empty($userDetail)) {
						...
						if (Auth::attempt([$loginType => Request::get('email'), 'password' => Request::get('password')])) {
							...
							return Redirect::intended('trainer/dashboard')->with('message', 'You are now logged in!');
						} else {
			...
			return View::make('trainer.login.index');
		}
	} // end index()
	...
	{
		return View::make('trainer.login.forget_password');
	} // end forgetPassword()
	/**
	...
	public function resetPassword($validate_string = null)
	{
		...
		if ($validate_string != "" && $validate_string != null) {
			...
			if (!empty($userDetail)) {
				return View::make('trainer.login.reset_password', compact('validate_string'));
			} else {
				return Redirect::to('trainer/')
					->with('error', trans('Sorry, you are using wrong link.'));
			}
		} else {
			return Redirect::to('trainer/')->with('error', trans('Sorry, you are using wrong link.'));
		}
	} // end resetPassword()
	/**
		...
		if ($validator->fails()) {
			...
			if (!empty($userDetail)) {
				if ($userDetail->is_active == 1) {
					...
					$route_url      	=  URL::to('trainer/reset_password/' . $forgot_password_validate_string);
					...
					return Redirect::to('trainer/forget_password')->with('error', trans('Your account has been temporarily disabled. Please contact traineristrator to unlock.'));
				}
			} else {
	...
	public function resetPasswordSave($validate_string = null)
	{
		...
		if ($validator->fails()) {
			return Redirect::to('trainer/reset_password/' . $validate_string)
				->withErrors($validator)->withInput();
		} else {
	...
	/**
	 * Function for logout trainer users
	 *
	 * @param null
	 *
	...
	public function logout()
	{
		...
		return Redirect::to('trainer/')->with('message', 'You are now logged out!');
	} //endLogout()
