 ...
 *
 * This file will render views\trainer\dashboard
 */
class AdminDashboardController extends BaseController
{
	/**
	 * Function for display trainer dashboard
	 *
	 * @param null
	 *
	...
	public function showdashboard()
	{
		...
		$totalTrainers			=	DB::table("users")->where("is_deleted", 0)->where("user_role_id", TRAINER_ROLE_ID)->where("is_email_verified", 1)->where("is_mobile_verified", 1)->get();

		$totalTrainees		=	DB::table("users")->where("is_deleted", 0)->where("user_role_id", TRAINEE_ROLE_ID)->where("is_email_verified", 1)->where("is_mobile_verified", 1)->get();
		...
		return  View::make('trainer.dashboard.dashboard', compact("totalTrainers", 'totalTrainees'));
	}
	/**
	 * Function for display trainer account detail
	 *
	 * @param null
	 *
	 * @return view page. 
	...
	{
		return  View::make('trainer.dashboard.myaccount');
	} // end myaccount()
	/**
	...
	public function bankdetail()
	{
		...
		return  View::make('trainer.dashboard.bankdetail', compact('userInfo'));
	} // end myaccount()
	/**
	...
	{
		return  View::make('trainer.dashboard.change_password');
	} // end myaccount()
	/**
	 * Function for update trainer account update
	 *
	 * @param null
	 *
	...
	public function myaccountUpdate()
	{
		...
		if ($validator->fails()) {
			return Redirect::to('trainer/myaccount')
			...
			if ($user->save()) {
				return Redirect::intended('trainer/myaccount')
	...
	/**
	 * 
	 * Function for update trainer bank detail update
	 *
	 * @param null
	 *
	 * @return redirect page. 
	...
	public function bankdetailUpdate()
	{
		...
		$validator 				= 	Validator::make(Request::all(), $ValidationRule);
		if ($validator->fails()) {
			return Redirect::to('trainer/bankdetail')
				->withErrors($validator)->withInput();
		} else {
			...
			if ($user->save()) {
				return Redirect::intended('trainer/bankdetail')
					->with('success', 'Information updated successfully.');
			}
		}
	} // end bankdetails()
	...
	public function changedPassword()
	{
		...
		if ($validator->fails()) {
			return Redirect::to('trainer/change-password')
				->withErrors($validator)->withInput();
		} else {
			$user 				= User::find(Auth::user()->id);
			$old_password 		= Request::get('old_password');
...
*/
	...
	{
		return View::make('trainer.user.user');
	}
