 ...
 */
class YourTrainingController extends BaseController
{
    public $model = 'YourTraining';
    ...
    public function userTrainings()
    {
        ...
        if (! empty($myTrainingsIds)) {
            ...
            $completed = Training::whereIn('trainings.id', $myTrainingsIds)->where('trainings.status', 1)->leftJoin('training_types', 'training_types.id', '=', 'trainings.type')->leftJoin('users', 'users.id', '=', 'trainings.user_id')->select('trainings.*', 'training_types.type as type', 'users.first_name as created_by')->orderBy('trainings.id', 'DESC')->get();
        ...
        return View::make("trainer.$this->model.userTraining", compact('ongoing', 'upcoming', 'completed', 'trainingResults', 'notifications'));
    }
    ...
    public function userTrainingReport()
    {
        ...
        foreach ($trainings as $training) {
            ...
            // dd($averageMinimumMark,$averageObtainMarks);
    ...
    public function training_details_popup($id)
    {
        ...
        // return $result;
        //   return $result;
        return View::make('trainer.YourTraining.popup', compact('result'));
    }
    ...
    public function userTrainingDetails($training_id = 0)
    {
        ...
        return View::make("trainer.$this->model.userTrainingDetails", compact('training_id', 'trainingDetails', 'trainingCourses', 'trainingQuestions', 'totalTrainees'));
    }
    ...
    public function userTrainingTest($training_id, $testId)
    {
        ...
        if (($alreadySubmittedTestResult && $testSubmitted && $alreadySubmittedTestResult->result == 'Passed')
            ...
            return View::make('trainer.YourTest.userTestSubmittedThanksPage', compact('testDetails', 'testAttendStatus'));
        } else {
            $trainingTest    = $test;
            ...
            if (! empty($questionsAlreadyAssigned)) {
                ...
                if ($trainingQuestions->isEmpty() && $trainingQuestions->count() < $testDetails->number_of_questions) {
                    ...
                    return back()->withErrors(['message' => 'Not enough questions available for this test. The test requires a minimum of ' . $testDetails->number_of_questions . ' questions. Please contact your trainer.']);
                }
            ...
            return View::make("trainer.$this->model.userTest", compact('training_id', 'trainingCoursesTitle', 'trainingDetails', 'trainingQuestions', 'totalTrainees', 'trainingTest', 'testDetails'));
        }
    }
    ...
    public function userTrainingTestResultOld($id)
    {
        ...
        return view('trainer.YourTraining.training-test-result', [
            'results'       => $results,
    ...
    public function userTrainingTestResult(Request $request, $id, $courseId)
    {
        ...
        if ($hasFreeTextQuestion) {
            return view('trainer.YourTraining.training-test-submit-freetext', [
    ...
    public function userTrainingCertificateDownload($id)
    {
        ...
        $pdf = PDF::loadView('trainer.YourTraining.certificate-pdf', $data);
