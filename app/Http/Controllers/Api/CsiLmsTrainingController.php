...
class RetailTrainingController extends BaseController
{
    ...
    public function userTrainingDetails(Request $request)
    {
        try {
            ...
            $myTrainingsIds = TrainingParticipants::where('trainee_id', $userId)->pluck('training_id')->toArray();
            //   echo '<pre>';
            // print_r( $myTrainingsIds );
            // die;
            if (!empty($myTrainingsIds)) {
                $trainings = Training::whereIn('trainings.id', $myTrainingsIds)->where('trainings.status', $request->status)->orderBy('trainings.id', 'DESC')->get();
            } else {
                $trainings = '';
            }
