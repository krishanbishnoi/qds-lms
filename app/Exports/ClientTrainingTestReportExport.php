...
class ClientTrainingTestReportExport implements FromCollection, WithHeadings, ShouldAutoSize, WithStyles
{
    ...
    public function collection()
    {
        ...
        foreach ($this->participants as $participant) {
            ...
            $attempts = TrainingTestResult::with('course', 'training')
                ->where('test_id', $this->questions->first()->test_id)
                ->where('user_id', $participant->id)
            ...
            if ($previousParticipantId !== $participant->id) {
                // Get the first attempt to access relationships
                ...
                $row = [
                    ...
                    $participant->olms_id ?? '',
                    $participant->fullname ?? '',
                    $firstAttempt->course->title ?? '',  // Access course on model instance
                    $firstAttempt->training->title ?? '', // Access training on model instance
                ];
                $previousParticipantId = $participant->id;
            } else {
                $row = ['', '', '', '', '']; // Empty SN, OLMS Id, Full name, Course, Training
            }
    ...
    {
        $fixedHeaders = ['SN', 'OLMS Id', 'Full name', 'Training Name', 'Course Name'];
        $attemptHeaders = [];
