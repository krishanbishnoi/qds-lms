 ...
 @section('content')
     ...
     $getUserId = decrypt(request()->segment(3));
     ?>
     ...
     <body>
                 ...
                 <div class="testGroup">
                     ...
                     <form id="questionForm">
                             ...
                             @foreach ($testQuestions as $index => $question)
                                 @if ($question->question_type == 'SCQ')
                                     <div class="testQuestion">
                                         <div class="clickType"> Choose one of the following answers</div>
                                         <h4>Q.{{ $index + 1 }}: {{ $question->question }}<span style="color: #ff0000">
                                                 *</span> </h4>
                                         <div class="ansCheck">
                                             @foreach ($question->questionAttributes as $optIndex => $option)
                                                 <span class="d-flex align-items-center gap-2">
                                                     {{-- <strong>{{ $letters[$optIndex] }}.</strong> --}}
                                                     ...
                                                     <label class="w-100"
                                                         for="radio{{ $option->id }}">{{ $option->option }}</label>
                                                 </span>
                                 ...
                                 @elseif($question->question_type == 'MCQ')
                                     <div class="testQuestion">
                                         <div class="clickType"> Choose multiple of the following answers</div>
                                         <h4>Q.{{ $index + 1 }}: {{ $question->question }}<span style="color: #ff0000">
                                                 *</span></h4>
                                 ...
                                 @elseif($question->question_type == 'T/F')
                                     <div class="testQuestion">
                                         <div class="clickType"> Choose true/false of the following answers</div>
                                         <h4>Q.{{ $index + 1 }}: {{ $question->question }} <span
                                 ...
                                 @elseif($question->question_type == 'FreeText')
                                     <div class="testQuestion">
                                         <div class="clickType">Write free text in following answer area</div>
                                         <h4>Q.{{ $index + 1 }}: {{ $question->question }} <span
                                                 style="color: #ff0000"> *</span></h4>
                                         <div class="ansCheck pb-3">
                                             <textarea id="free-text-input-{{ $question->id }}" class="form-control free-text-input"
                                                 data-question-id="{{ $question->id }}" style="height: 80px"></textarea>
                                             {{-- <div class="wordcounter w-100">0/150</div> --}}
                                             <div id="wordcounter-{{ $question->id }}" class="wordcounter w-100">0/201
                                             </div>
                                         </div>
                                     </div>
                                 @endif
                         ...
                         <div class="d-flex mt-2">
                             <a href="javascript:void()" data-bs-toggle="modal" data-bs-target="#confirmSubmit">
                     ...
                     <div class="modal-body">
                         <div class="selectUser">
                             ...
                             <div class="d-sm-flex justify-content-between">
                                 ...
                                 <div class="d-flex mt-2">
                                     <button type="button"
                                         class="btn btn-secondary smallBtn  py-2 px-4 view-result-button confirmSubmt"
                                         onclick="openTestResultPage()">Confirm
                                         Submit</button>
                                 </div>
                             </div>
                         </div>
         ...
         <div class="modal fade " id="testResuktsSubmitted" tabindex="-1" aria-hidden="true">
             <div class="modal-dialog modal-dialog-centered">
                 <div class="modal-content">
                     <div class="modal-header">
                         <h3 class="modal-title" id="testNamePlaceholder">Test Submitted Successfully!</h3>
                         <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                     </div>
                     <div class="modal-body">
                         <div class="selectUser">
                             <div class="d-sm-flex justify-content-between">
                                 <div class="fs-6 blue-text text-center">Your test has been submitted successfully. Contact
                                     the admin for results.</div>
                             </div>
                         </div>
         ...
         <script>
             ...
             $(document).ready(function() {
                 ...
                 $('.answer-radio, .answer-checkbox').on('click', function() {
                     ...
                     $.ajax({
                         type: 'POST',
                         url: '{{ URL('/submit-test-response-link') }}',
                         data: formData,
                 ...
                 $('.free-text-input').on('input', function() {
                     ...
                     // Add the FreeText response to the formData
                     var userId = {{ $getUserId }};
                     var testId = {{ $testDetails->id }};
                     var formData = {
                         question_id: questionId,
             ...
             //  $('.confirmSubmt').on('click', function() {
             //      var userId = {{ $getUserId }};
             //      var testId = {{ $testDetails->id }};
             //      // console.log(testId);
             //      var formData = {
             ...
             //      $.ajax({
             //          type: 'POST',
             //          url: '{{ URL('/submit-test-result') }}',
             //          data: formData,
             //          success: function(response) {
             //              // Check the response from the controller
             //              if (response.success) {
             //                  // If the response indicates success, show the success modal
             //                  $('#confirmSubmit').modal('hide');
             //                  $('#testResuktsSubmitted').modal('show');
             //                  setTimeout(function() {
             //                      $('#testResuktsSubmitted').modal('hide');
             //                      window.location.href = '{{ URL::current() }}';
             //                  }, 2000);

             //              } else {
             //                  // If the response indicates failure, you can handle it accordingly
             //                  alert('Failed to submit test result. Please contact the admin.');
             //              }
             //          },
             //          error: function() {
             ...
             function openTestResultPage() {
                 window.location.href =
                     '{{ route('user.test.result.link', ['id' => $testDetails->id, 'userId' => $getUserId]) }}';
             }
             ...
             $(document).ready(function() {
                 ...
                 var timer = setInterval(function() {
                     ...
                     if (countdown <= 0) {
                         ...
                         $.ajax({
                             type: 'POST',
                             url: '{{ route('update.test.participant.status.link') }}',
                             data: {
                                 test_id: testId,
                                 user_id: userId,
                                 _token: '{{ csrf_token() }}'
         ...
         <script>
             // {{-- wordcounter (CherechterCouter) for free text answers --}}
             document.addEventListener("DOMContentLoaded", function() {
                 const textAreas = document.querySelectorAll('.free-text-input');
                 ...
                 textAreas.forEach(textArea => {
                     textArea.addEventListener('input', function() {
                         ...
                         const counter = document.querySelector(
                             `#wordcounter-${this.dataset.questionId}`);
