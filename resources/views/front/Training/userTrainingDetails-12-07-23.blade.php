...
@section('content')
            ...
            <div class="d-flex flex-wrap align-items-center">
                ...
                <div class="courseProgress">
                    {{-- <div class="progress-main">
                        <div class="progress-circle" data-progress="90"><img src="{{ asset('front/img/processing.svg') }}"
    ...
    <div class="d-flex flex-wrap paddingTop">
        ...
        <div class="abouttraining">
            ...
            <div class="tab-content imgwrapper" id="nav-tabContent1">
                @foreach ($trainingCourses as $course)
                    @foreach ($course->CourseContentAndDocument as $content)
                        ...
                        @elseif ($content['type'] === 'doc')
                            @php
                                $documentCompletion = \App\Models\TrainingDocumentCompletion::where('user_id', Auth::user()->id)
                            ...
                            <div class="tab-pane courseDocument{{ $content['id'] }}"
                                ...
                                <div class="imgwrapper">
                                    ...
                                    <!-- Container for PDF.js rendering -->
                                    <div id="pdf-container{{ $content['id'] }}"></div>
                                    <!-- Placeholder for the checkbox -->
                                    <div style="background: #525659; margin-top: -5px; padding: 8px; text-align: center;">
                                        <span>
                    ...
                    @foreach ($trainingCourses as $index => $course)
                        <div class="accordion-item">
                            ...
                            <div id="collapse{{ $index + 1 }}" class="accordion-collapse collapse"
                                    ...
                                    @foreach ($course->CourseContentAndDocument as $key => $content)
                                        ...
                                        <li>
                                            <a href="#courseViseView{{ $content['id'] }}" class="courseGroup"
                                                data-type="{{ $content['type'] }}" data-content-id="{{ $content['id'] }}"
                                                data-video-duration="{{ $content['length'] }}"
                                                data-pdf-pages="{{ $content['length'] }}"
                                                onclick="handleDocumentClick({{ $key }}, {{ $content['id'] }}, {{ json_encode($documentCompletion) }})">
                                                ...
                                                <span>
                                                    ...
                                                    @elseif($content['type'] === 'doc')
                                                        <img class="me-2" src="{{ asset('front/img/pdficon.svg') }}"
                                                            width="18" height="18">PDF
                                                    @endif
                                                </span>
                                            </a>
                                            @if (!empty($course->test_id))
                                                @php
                                                    $testAlreadySubmited = \App\Models\TrainingTestParticipants::where('training_id', $course->training_id)
                                                ...
                                                @else
                                                    <a class="btn btn-secondary py-2"
                                                        href="{{ route('userTraining.test', ['training_id' => $training_id, 'test_id' => $course->test_id]) }}">Begin
                                                        Test</a>
                                                @endif
                                            @endif
                                        </li>
                                ...
                                <script>
                                    function handleDocumentClick(index, documentId, documentCompletion) {
                                        if (index === 0 || (documentCompletion && documentCompletion.status === 1)) {
                                            var documentDiv = document.getElementById('courseViseView' + documentId);
                                            if (documentDiv) {
                                                documentDiv.style.setProperty('display', 'none', 'important');
                                            }
                                        } else {
                                            alert('Please read the previous document first.');
                                            var documentDiv = document.getElementById('courseViseView' + documentId);

                                            if (documentDiv) {
                                                console.warn(documentDiv);
                                                documentDiv.style.setProperty('display', 'none', 'important');
                                            }
                                        }
                                    }
