                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                            ...
                            <input type="text" id="name" name="title" class="form-control" value="{{ old('title', isset($Tags) ? $Tags->title : '') }}" required>
                            </div>
                            @if($errors->has('name'))
                    ...
                    <div class="col-md-12">
                        <div class="form-group {{ $errors->has('from_email') ? 'has-error' : '' }}">
                            ...
                            <input type="text" id="name" name="discount" class="form-control" value="{{ old('name', isset($Tags) ? $Tags->discount : '') }}" required>
                            </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group {{ $errors->has('from_email') ? 'has-error' : '' }}">
                            @php
                            $discounttag = explode(",", $Tags->user_id);
                            @endphp
                            ...
                            <select class="form-control select2" name="user[]" multiple>
                                ...
                                <option value="{{$user->id}}" {{isset($discounttag) && in_array($user->id, $discounttag) ? 'selected' : ''}}>{{$user->name}}</option>
                                @endforeach
                            </select>
                            @if($errors->has('from_email'))
                                <em class="invalid-feedback">
