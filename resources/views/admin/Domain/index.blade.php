            ...
            <div class="box-body">
                ...
                <div class="col-md-2 col-sm-2">
                    <div class="form-group ">
                        {!! Html::decode(
                            Form::label('domain', trans('Domain') . '<span class="requireRed"> </span>', ['class' => 'mws-form-label']),
                        ) !!}
                        {{ Form::text('domain', isset($searchVariable['domain']) ? $searchVariable['domain'] : '', ['class' => 'form-control', 'placeholder' => 'Category']) }}
                    </div>
                </div>
                                ...
                                <tr>
                                    <th width="25%">
                                        {{ link_to_route(
                                            ...
                                            [
                                                'sortBy' => 'domain',
                                                'order' => $sortBy == 'domain' && $order == 'desc' ? 'asc' : 'desc',
                                                $query_string,
                                            ],
                                            [
                                                'class' =>
                                                    $sortBy == 'domain' && $order == 'desc'
                                                        ? 'sorting desc'
                                                        : ($sortBy == 'domain' && $order == 'asc'
                                                            ? 'sorting asc'
                                                            : 'sorting'),
                                            ],
                                        ) }}
                                ...
                                @if (!$results->isEmpty())
                                    @foreach ($results as $record)
                                        <tr class="items-inner">
                                            <td data-th='{{ trans('Page Name') }}'>{{ $record->domain }}</td>
